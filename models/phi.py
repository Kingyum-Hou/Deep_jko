...
class Model(nn.Module):
    def __init__(self, args, r=10):
        ...
        self.nTh  = args.num_layers
        self.d    = args.space_dim
        #self.alph = args.alphas

        r = min(r,self.d+1) # if number of dimensions is smaller than default r, use that

        self.A  = nn.Parameter(torch.zeros(r, self.d+1) , requires_grad=True)
        self.A  = nn.init.xavier_uniform_(self.A)
        self.c  = nn.Linear( self.d+1  , 1  , bias=True)  # b'*[x;t] + c
        self.w  = nn.Linear( self.m    , 1  , bias=False)

        self.N = ResNN(self.d, self.m, nTh=self.nTh)

        # set initial values
        self.w.weight.data = torch.randn(self.w.weight.data.shape)
        self.c.weight.data = torch.randn(self.c.weight.data.shape)
        self.c.bias.data   = torch.randn(self.c.bias.data.shape)
    ...
    def gradAndHessian(self,x, justGrad=False ):
        """
